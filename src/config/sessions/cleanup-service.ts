...
import { loadSessionStore, type SessionMaintenanceApplyReport } from "./store.js";
import {
...
export async function runSessionsCleanup(params: {
  ...
  if (!opts.dryRun) {
    for (const target of targets) {
      ...
      missingApplied = missingRemovals.filter(({ sessionKey }) =>
        removedSessionKeys.has(sessionKey),
      ).length;
      dmScopeRetiredApplied = dmScopeRetiredRemovals.filter(({ sessionKey }) =>
        removedSessionKeys.has(sessionKey),
      ).length;
      ...
      const appliedReport =
        lifecycleResult.maintenanceReport as SessionMaintenanceApplyReport | null;
      const summary: SessionCleanupSummary =
        appliedReport === null
          ? {
              ...(preview?.summary ?? {
